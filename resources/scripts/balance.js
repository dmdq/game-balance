...
function login(obj){
	...
	$(obj).attr('disabled','disabled').html("登陆..").addClass("action_btn_disabled");
	$.get("./?method=loadUser&args="+name+"&args2="+password,function(data){
		$(obj).removeAttr("disabled").html("登陆").removeClass("action_btn_disabled");
		if(typeof(data)!='object'){
			data = eval("("+data+")");
		}
		var code = data.code;
...
function register(obj){
		...
		$(obj).attr('disabled','disabled').html("注册..").addClass("action_btn_disabled");
		$.get("./?method=addUser&args="+name+"&args2="+password,function(data){
			...
			var code = data.code;
			if(1==msgcode(code)){
				remember("balance.user.name",true,"rem_password");
				remember("balance.user.password",true,"rem_password");
				login();
			}
			$(obj).removeAttr("disabled").html("注册").removeClass("action_btn_disabled");
		});
}
/**
 * 添加应用
...
function push(obj){
	...
	if(null!=uuid&&uuid!=""){
		var url  = "http://127.0.0.1:8888/root.html#sid="+uuid;
		console.log(url);
		window.open(url);
	}
}
...
function add_item_bydata(target,data){
	...
	if(data.status){
		...
		html += status==99?'<option value="99" selected="selected">新服</option>':'<option value="99">新服</option>';
		html += status==100?'<option value="100" selected="selected">正常</option>':'<option value="100">正常</option>';
		html += status==101?'<option value="101" selected="selected">推荐</option>':'<option value="101">推荐</option>';
	...
	if(data.version){
		html += '<input type="text" class="version" value="'+data.version+'" onFocus="extend_version(this)" onblur="close_version(this);" />';
	}
	...
	if(target.parent().attr("id")=="apps"){
		html += '<button title="浏览" onclick="browse(this);" class="browse" style="display:none;">浏览</button>';
	}else{
		html += '<button title="浏览" onclick="browse(this);" class="browse">浏览</button>';
	}
	if(data.version){
		html += '<button title="广播" onclick="push(this);" class="push">广播</button>';
	}
	html += '</div>';
	html += "</li>";
	target.append(html);
...
function add_process(){
	...
	var reg2 = /^([0,1]?\d{0,2}|2[0-4]\d|25[0-5])\.([0,1]?\d{0,2}|2[0-4]\d|25[0-5])\.([0,1]?\d{0,2}|2[0-4]\d|25[0-5])\.([0,1]?\d{0,2}|2[0-4]\d|25[0-5])$/;  
    ...
    var flag = reg.test(host)|reg2.test(host);
	if(!flag){
		msgbox("主机地址格式异常");
		return;
	} 
...
function readCookie(cookieName,id,clickId){
	 var cookie_name= cookieName; 
	 var clear = true;
	 if($.cookie(cookie_name)){
	 	$("#"+id).val($.cookie(cookie_name));
	 	$("#"+clickId).attr("checked",true);
	 	clear &=false;
	 }
	// remember(cookieName,clear,id,clickId);
}
/**
 * cookie记住 
 */
function remember(cookie_name,id,clickId){
	var checkedObj =  $("#"+clickId);
	checkedObj.click(function(){
		  if(this.checked){
			checkedObj.attr("checked",true);			  
		    $.cookie(cookie_name,$("#"+id).val(),{path:'/',expires:100});
		  }else{
			  checkedObj.attr("checked",false);			  
		  	$.cookie(cookie_name,null,{path:'/'});
		  }
	 });
	if(checkedObj.attr("checked")){
		  $.cookie(cookie_name,$("#"+id).val(),{path:'/',expires:100});
	}else{
		$.cookie(cookie_name,null,{path:'/'});
	}
